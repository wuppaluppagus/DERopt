if ~isempty(el_v) || ~isempty(rel_v)
    if ~isempty(el_v)
        for i = 1:size(el_v,2)
            Constraints = [Constraints
                (0 <= var_el.el_adopt):'Electrolyzer capacity >0'
                (0 <= var_el.el_prod):'Electrolyzer production >0'
    ...
    if  ~isempty(rel_v)
        for i = 1:size(rel_v,2)
            Constraints = [Constraints
                (0 <= var_rel.rel_adopt):'Electrolyzer capacity >0'
                (0 <= var_rel.rel_prod):'Electrolyzer production >0'
...
if ~isempty(el_binary_v)
    for i = 1:size(el_binary_v,2)
        Constraints = [Constraints
            (0 <= var_el_binary.el_adopt):'Electrolyzer - Binary model adoption >0'
            (0 <= var_el_binary.el_prod):'Electrolyzer - Binary model output >0'
            (0 <= var_el_binary.el_onoff):'Electrolyzer - Binary model state >0'
...
if ~isempty(h2es_v)
    for ii = 1:size(h2es_v,2)
        Constraints = [Constraints
            (0 <= var_h2es.h2es_adopt):'H2ES Capacity is greater than zero'
            (0 <= var_h2es.h2es_soc):'H2ES SOC is greater than zero'
            (0 <= var_h2es.h2es_chrg):'H2ES charging is greater than zero'
            (0 <= var_h2es.h2es_dchrg):'H2ES discharging is greater than zero'
            (0 <= var_h2es.h2es_bin):'H2ES charging state is greater than zero'
